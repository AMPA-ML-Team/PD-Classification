...
def MLP_PD(params):
    ...
    for train_idx, test_idx in folds.split(X):
        ...
        mlp = MLPClassifier(solver=params['solver'], alpha=params['alpha'],
                            hidden_layer_sizes=(params['first_layer'],params['second_layer']),
                            random_state=params['random_state'], max_iter=params['max_iter'],
                            activation=params['activation'], max_fun=params['max_fun'])
...
def running_MLP(params):
    best_result = 0
    best_fscore = 0
    sum = 0
    number_of_runs = params['number_of_runs']
    for i in range(number_of_runs):
        scores, confs = MLP_PD(params)
